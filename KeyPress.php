...
class WebDriverDemo extends Sauce\Sausage\WebDriverTestCase
{
    ...
    {
        $this->url('https://s3-ap-northeast-1.amazonaws.com/logn.in/keypress.html');
    }
    ...
    public function testKeyPress()
    {
        ...
        for ($i = 0; $i <= 255; $i++)
        {
            ...
            foreach (split("<br/>", $text) as $index => $line)
            {
                ...
                file_put_contents('logs/keypress_'.$this->getBrowser().'_'.$this->getDesiredCapabilities()['version'].'_'.$this->getDesiredCapabilities()['platform'], $line, FILE_APPEND);
            }
